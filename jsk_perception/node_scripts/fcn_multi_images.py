...
class FCNObjectSegmentation(ConnectionBasedTransport):
    ...
    def _load_torch_model(self):
        ...
        if model_name == 'fcn8s_at_once':
            self.model = torchfcn.models.FCN8sAtOnce(n_class=n_class)
    ...
    def subscribe(self):
        multi_image = rospy.get_param('~multi_image', True)
        if multi_image:
            queue_size = rospy.get_param('~queue_size', 10)
            sub_img = message_filters.Subscriber(
                '~input', Image, queue_size=1, buff_size=2**24)
            sub_img2 = message_filters.Subscriber(
                '~input', Image, queue_size=1, buff_size=2**24)
            sub_img3 = message_filters.Subscriber(
                '~input', Image, queue_size=1, buff_size=2**24)
            sub_img4 = message_filters.Subscriber(
                '~input', Image, queue_size=1, buff_size=2**24)
            self.subs = [sub_img, sub_img2, sub_img3, sub_img4]
